...
# slows the edit loop. It NEVER runs npm test or the full build:
...
# The heavier deterministic floor (fast pure unit tests) runs once per push in
# .githooks/pre-push.
#
...
#   - a console.log call added to a server file under lib/ (server code must use
#     @reldens/utils Logger, never the console).
...
# Added lines in this working tree, across source and docs. Lockfiles and package-lock.json
# are excluded (generated, never hand-checked); node_modules and dist builds are gitignored
...
out=$(printf '%s' "$stream" | perl -CSD -e '
  ...
  my $body = "QA stop-gate blocked: $n line(s) this change added violate a hard project invariant. "
    ...
    . "disables a test suite; no leftover debugger statement; no console.log in server files under "
    . "lib/ - use @reldens/utils Logger):";
  $body .= "\n- $_" for @hits;
  $body =~ s/([\\"])/\\$1/g;
