...
#
# Reldens ships a pre-push QA floor in .githooks/. Git does not use a checked-in
...
if [ -z "$current" ]; then
  if git config --local core.hooksPath .githooks 2>/dev/null; then
    echo "Reldens: enabled .githooks (pre-push QA floor: fast pure unit tests + copy guards). Undo with: git config --unset core.hooksPath" >&2
  fi
else
  ...
  case "$current" in
    /*/.githooks)
      if [ "$current" != "$dir/.githooks" ]; then
        echo "Reldens: core.hooksPath points at another checkout's .githooks ($current); the pre-push floor that runs here may be a stale copy. Repoint with: git config --local core.hooksPath .githooks" >&2
      fi
      ;;
  esac
fi
