...
const { createMsgRateBucket, consumeInboundFrame, tallyDrop, MSG_RATE_KICK_REASON } = require('./msg-rate-limit');
const { createMsgLanes, classifyMsgLane, consumeLaneToken } = require('./msg-lanes');
...
class RoomLogin extends Room
{
    ...
    onCreate(options)
    {
        ...
        this.onMessage('*', this.handleReceivedMessageGate.bind(this));
    ...
    removeActivePlayer(playerSchema, client, isChangingScene)
    {
        ...
        this.msgBuckets?.delete(playerSchema.sessionId);
    }

    /**
     * Per-connection inbound rate gate: runs BEFORE the room message handler
     * (which subclasses override as handleReceivedMessage). Verdicts are
     * allow / drop / kick — a kick closes the socket with the wire-contract
     * reason, a drop is silent throttling. Buckets are keyed by sessionId and
     * created lazily; a size cap sweeps stale entries.
     *
     * @param {Client} client
     * @param {Object} message
     * @returns {Promise<void>}
     */
    async handleReceivedMessageGate(client, message)
    {
        if(!this.msgBuckets){
            this.msgBuckets = new Map();
        }
        let nowSec = Date.now() / 1000;
        let bucket = this.msgBuckets.get(client.sessionId);
        if(!bucket){
            bucket = createMsgRateBucket(nowSec);
            bucket.msgLanes = createMsgLanes(nowSec);
            this.msgBuckets.set(client.sessionId, bucket);
            if(1000 < this.msgBuckets.size){
                // sweep: drop the oldest half of the tracked sessions
                let keys = this.msgBuckets.keys();
                for(let i = 0; i < 500; i++){
                    this.msgBuckets.delete(keys.next().value);
                }
            }
        }
        let approxBytes = 1024;
        try {
            approxBytes = JSON.stringify(message).length;
        } catch (error) {
            // non-serializable message: count a fixed cost
        }
        const verdict = consumeInboundFrame(bucket, nowSec, approxBytes);
        if('kick' === verdict.verdict){
            this.msgBuckets.delete(client.sessionId);
            Logger.warning('Message rate kick: '+client.sessionId, verdict);
            client.leave(1008, MSG_RATE_KICK_REASON);
            return;
        }
        if('drop' === verdict.verdict){
            return;
        }
        // Post-parse per-class lane gate: classify the (already parsed)
        // message and spend a lane token. Exempt frames never consume lane
        // tokens. A lane drop tallies into the gate's shared per-second abuse
        // window (tallyDrop) so a lane flood kicks through the identical
        // verdict path as a pre-parse flood.
        const lane = classifyMsgLane(message);
        if('exempt' !== lane && 'drop' === consumeLaneToken(bucket.msgLanes, lane, nowSec)){
            const laneVerdict = tallyDrop(bucket, nowSec);
            if('kick' === laneVerdict){
                this.msgBuckets.delete(client.sessionId);
                Logger.warning('Message rate kick: '+client.sessionId, {cause: 'lane'});
                client.leave(1008, MSG_RATE_KICK_REASON);
            }
            return;
        }
        return await this.handleReceivedMessage(client, message);
    }
