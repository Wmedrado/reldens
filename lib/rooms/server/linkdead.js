 ...
 * grace; they tear down through the room leave flow directly. This module is
...
/** @type {number} */
const LINKDEAD_GRACE_MS = 5 * 60 * 1000;

/**
 * @typedef {Object} LinkdeadSessionView
 * @property {number} accountId
 * @property {boolean} linkdead
 * @property {boolean} left
 *   True once the room leave flow has begun tearing the session down. leave()
 *   sets it synchronously, then awaits the character save before removing the
 *   entity and releasing the character load lease; a session in that window
 *   must never be resumed (the reconnect would get a zombie whose lease
 *   release the nonce fence cannot see, since the resume arm never
 *   re-acquires).
 * @property {boolean} escrowQuarantined
 *   True once this session's guild bank escrow was ROLLED BACK: its live state
 *   was abandoned and it can never persist again. RESUMING it would hand the
 *   player a session that plays normally and saves NOTHING, forever, with no
 *   error and no signal: silent unbounded data loss, which is far worse than
 *   the reconnect refusal below. The refused client retries into the
 *   fresh-join arm and loads from its durable row, which is exactly what the
 *   rollback intends.
 */

/**
 * @typedef {{action: 'resume'}|{action: 'reject', error: string}|{action: 'join'}} JoinPlan
 */
