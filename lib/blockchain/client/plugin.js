...
const { WalletLinkUI } = require('./wallet-ui');
const { HolderTierBadge } = require('./tier-badge');
const { BlockchainSnippets } = require('./snippets/en_US');
...
class BlockchainClientPlugin extends PluginInterface
{
    ...
    async setup(props)
    {
        ...
        this.setupWalletLinkUi();
        this.setupTierBadge();
        this.listenEvents();
    }

    /**
     * Register the blockchain client snippets into the client config so the
     * translator can resolve the "blockchain" namespace.
     *
     * @returns {boolean}
     */
    setTranslations()
    {
        ...
        TranslationsMapper.forConfig(this.gameManager.config.client, BlockchainSnippets);
        return true;
    }
    ...
    getTranslator()
    {
        return this.gameManager?.services?.translator || false;
    }
    ...
    setupWalletLinkUi()
    {
        this.walletLinkUI = new WalletLinkUI({
            gameManager: this.gameManager,
            events: this.events,
            config: this.gameManager?.config || false,
            translator: this.getTranslator(),
            walletManager: this.walletManager
        });
        this.walletLinkUI.render();
        // after a successful post-login link, refresh the holder tier badge:
        this.walletLinkUI.onLinkedCallback = (result) => {
            this.tierBadge.update(
                result?.walletHolder || this.gameManager?.initialGameData?.walletHolder || false
            );
        };
        return true;
    }
    ...
    setupTierBadge()
    {
        this.tierBadge = new HolderTierBadge({
            gameManager: this.gameManager,
            events: this.events,
            translator: this.getTranslator()
        });
    ...
    listenEvents()
    {
        ...
        this.events.on('reldens.beforeJoinGame', async (event) => {
            ...
            // informational: attach the connected wallet address even without a
            // pending challenge, so the game room receives it in the options.
            this.attachConnectedAddress(event);
        });
        return true;
    }

    /**
     * Attach the connected wallet address to the join options. A wallet alone
     * is not enough to link the account (a signed server challenge is required,
     * see the WalletLinkUI link-after-login flow), but the address is useful
     * information for the game room and the wallet UI.
     *
     * @param {Object} event
     * @returns {boolean}
     */
    attachConnectedAddress(event)
    {
        ...
        if(this.gameManager?.userData){
            this.gameManager.userData.walletAddress = walletState.address;
        }
        return true;
    }
