/**
 *
 * Reldens - AchievementsEntityOverride
 *
 * Extends the achievements entity with custom property aliases for the admin
 * panel.
 *
 */
...
class AchievementsEntityOverride extends EntityProperties
{
    ...
    static propertiesConfig(extraProps)
    {
        ...
        config = this.updateProperty(config, 'type', 'label', 'Type (kill | gather | craft | quest)');
        config = this.updateProperty(config, 'target_key', 'label', 'Target Key (empty = any)');
        config = this.updateProperty(config, 'reward_item_id', 'alias', 'achievement_reward_item');
        return config;
    }
...
module.exports.AchievementsEntityOverride = AchievementsEntityOverride;
