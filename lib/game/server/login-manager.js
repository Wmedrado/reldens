...
const { AuthThrottle } = require('../../users/server/auth-throttle');
const { Totp } = require('../../users/server/totp');
const { IpBlock } = require('../../users/server/ip-block');
const { IpBlockDb } = require('../../users/server/ip-block-db');
const { Turnstile } = require('../../users/server/turnstile');
...
class LoginManager
{
    ...
    ipBlockEnabled()
    {
        if(this._ipBlockEnabled === undefined){
            this._ipBlockEnabled = Boolean(
                this.config.getWithoutLogs('server/users/ipBlockEnabled', false)
                || Number(process.env.RELDENS_IP_BLOCK_ENABLED || 0)
            );
        }
        return this._ipBlockEnabled;
    }

    /**
     * Hydrate the in-memory blocklist from the blocked_ips storage entity once.
     * Graceful when the data server or entity is unavailable: the gate stays open.
     * @returns {Promise<void>}
     */
    async ensureIpBlockList()
    {
        if(this.ipBlockList){
            return;
        }
        let dataServer = this.usersManager?.dataServer;
        if(!dataServer){
            return;
        }
        try {
            let ipBlockDb = new IpBlockDb(dataServer);
            let entries = await ipBlockDb.loadActiveBlockedIps();
            this.ipBlockList = new IpBlock.IpBlockList();
            this.ipBlockList.setEntries(entries);
        } catch (error) {
            Logger.error('IP block list load error.', error.message);
        }
    }

    /**
     * Returns true when the request IP is blocked. Only active when ipBlockEnabled
     * and the client actually sent an IP (options.ip); otherwise the gate stays open.
     * @param {Object<string, any>} userData
     * @returns {Promise<boolean>}
     */
    async checkIpBlocked(userData)
    {
        if(!this.ipBlockEnabled()){
            return false;
        }
        let ip = userData?.ip;
        if(!ip){
        ...
        await this.ensureIpBlockList();
        if(!this.ipBlockList){
            return false;
        }
        if(this.ipBlockList.isBlocked(IpBlock.cleanIp(ip), Date.now())){
            Logger.warning('Blocked IP login attempt.', ip, userData?.username);
            return true;
        }
        return false;
    }
    ...
    turnstileEnabled()
    {
        if(this._turnstileEnabled === undefined){
            this._turnstileEnabled = Boolean(this.config.getWithoutLogs('server/users/turnstileEnabled', false));
        }
        return this._turnstileEnabled;
    }

    /**
     * Returns true when the request may proceed. Off when the config flag is off or
     * no RELDENS_TURNSTILE_SECRET is configured (local dev / tests).
     * @param {Object<string, any>} userData
     * @returns {Promise<boolean>}
     */
    async passesTurnstileGate(userData)
    {
        if(!this.turnstileEnabled()){
            return true;
        }
        let secret = process.env.RELDENS_TURNSTILE_SECRET;
        if(!secret){
            return true;
        }
        return await Turnstile.passesTurnstile({
            token: userData?.turnstileToken,
            secret,
    ...
    async login(user, userData)
    {
        ...
        // check the IP blocklist (only when enabled and an IP was sent):
        if(await this.checkIpBlocked(userData)){
            Logger.warning('Login rejected: blocked IP.', userData?.username);
            return {error: 'Your IP address is currently blocked. Please try again later.'};
        }
        // check the turnstile token (only when enabled and a secret is configured):
        if(!await this.passesTurnstileGate(userData)){
            Logger.warning('Login rejected: turnstile verification failed.', userData?.username);
            return {error: 'Could not verify you are not a bot. Please try again.'};
        }
        ...
        if(!this.passwordManager.validatePassword(userData.password, user.password)){
            ...
            AuthThrottle.recordAuthFailure(user.username);
            this.events.emitSync('reldens.loginInvalidPassword', this, user, userData, result);
            return result;
        }
        // check the two-factor code when the user has TOTP enabled:
        if(Number(user.totp_enabled) === 1){
            let totpCode = userData?.totpCode;
            if(!user.totp_secret || !totpCode || null === Totp.verifyTotp(user.totp_secret, totpCode, Date.now())){
                Logger.warning('Invalid two-factor code for user login.', userData?.username);
                AuthThrottle.recordAuthFailure(user.username);
        ...
        try {
            ...
            AuthThrottle.clearAuthFailures(user.username);
    ...
    async register(userData)
    {
        ...
        // check the IP blocklist (only when enabled and an IP was sent):
        if(await this.checkIpBlocked(userData)){
            Logger.warning('Registration rejected: blocked IP.', userData?.username);
            return {error: 'Your IP address is currently blocked. Please try again later.'};
        }
        // check the turnstile token (only when enabled and a secret is configured):
        if(!await this.passesTurnstileGate(userData)){
            Logger.warning('Registration rejected: turnstile verification failed.', userData?.username);
            return {error: 'Could not verify you are not a bot. Please try again.'};
        }
        try {
