...
class InventoryMessageActions
{
    ...
    async executeEquipAction(playerSchema, data)
    {
        ...
        if(!item.equipped){
            // Respect the group items_limit so multiple pieces of the same group
            // can be worn at once (e.g. 2 rings) instead of the old one-per-group.
            let groupLimit = this.getGroupItemsLimit(playerSchema.inventory.manager, item.group_id);
            let equippedInGroup = this.countEquippedInGroup(playerSchema.inventory.manager.items, item.group_id);
            if(equippedInGroup >= groupLimit){
                if(groupLimit <= 1){
                    // Single-slot group: swap the previous piece out (original behaviour).
                    this.unEquipPrevious(item.group_id, playerSchema.inventory.manager.items);
                } else {
                    Logger.info('Cannot equip item, group slot limit reached ('+groupLimit+'): '+item.getInventoryId());
                    return false;
                }
            }
            await item.equip();
     ...
     * @returns {number}
     */
    getGroupItemsLimit(manager, groupId)
    {
        let groupModel = (manager.groups?.groupModels || []).find((g) => g.id === groupId);
        let limit = groupModel ? Number(groupModel.items_limit) : NaN;
     ...
     * @returns {number}
     */
    countEquippedInGroup(itemsList, groupId)
    {
        let count = 0;
        for(let i of Object.keys(itemsList)){
            if(itemsList[i].group_id === groupId && itemsList[i].equipped){
                count++;
            }
        }
        return count;
