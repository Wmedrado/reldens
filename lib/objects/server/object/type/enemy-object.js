...
class EnemyObject extends NpcObject
{
    ...
    constructor(props)
    {
        ...
        this.roaming = sc.get(props, 'roaming', this.config.getWithoutLogs('server/enemies/roaming', false));
        this.roamingRadius = Number(
            sc.get(props, 'roamingRadius', this.config.getWithoutLogs('server/enemies/roamingRadius', 3))
        );
        this.aggroRange = Number(
            sc.get(props, 'aggroRange', this.config.getWithoutLogs('server/enemies/aggroRange', 0))
        );
        this.switchTargetOnHit = sc.get(
            props,
            'switchTargetOnHit',
            this.config.getWithoutLogs('server/enemies/switchTargetOnHit', false)
        );
        this.roamingTimer = false;
    ...
    async runAdditionalRespawnSetup()
    {
        ...
        await this.loadDamageTypes();
        this.setupAggressiveBehavior();
        this.events.onWithKey(
            'reldens.sceneRoomOnCreate',
            this.setupCombatAi.bind(this),
            this.eventUniqueKey('setupCombatAi'),
    ...
    /**
     * Setup proximity aggro and roaming once the room is created.
     *
     * @param {RoomScene} room
     */
    setupCombatAi(room)
    {
        if(0 < this.aggroRange){
            this.attachProximityAggro(room);
        }
        if(this.roaming){
            this.roamingRoom = room;
            this.startRoaming(room);
        }
    }

    /**
     * Aggro any player that enters the configured aggro range, regardless of
     * the respawn area (adapted from Kaetram aggroRange).
     *
     * @param {RoomScene} room
     */
    attachProximityAggro(room)
    {
        let listener = (event) => {
            ...
            for(let body of event.target.bodies){
                ...
                if(!body.position || !this.objectBody?.position){
                    continue;
                }
                let distX = body.position[0] - this.objectBody.position[0];
                let distY = body.position[1] - this.objectBody.position[1];
                let distance = Math.sqrt(distX * distX + distY * distY);
                if(distance > this.aggroRange){
                    continue;
                }
                this.startBattleWithPlayer({bodyA: body, room});
                return;
            }
        };
        this.postBroadPhaseListener.push(listener);
        room.roomWorld.on('postBroadphase', listener);
    }

    /**
     * Roam towards random walkable nodes inside the roaming radius while idle
     * (adapted from Kaetram mob roaming).
     *
     * @param {RoomScene} room
     * @returns {boolean}
     */
    ...
    roamTick(room)
    {
        ...
        if(this.objectBody.bodyState?.inState && GameConst.STATUS.DEATH === this.objectBody.bodyState.inState){
            return false;
        }
        let walkableNodes = WorldWalkableNodesAroundProvider.generateWalkableNodesAround(
            this.objectBody,
            room.roomWorld?.pathFinder
        );
        if(!walkableNodes || 0 === walkableNodes.length){
            return false;
        }
        let candidates = walkableNodes.filter((node) => {
            let colDiff = Math.abs(node.col - this.objectBody.currentCol);
            let rowDiff = Math.abs(node.row - this.objectBody.currentRow);
            return colDiff <= this.roamingRadius && rowDiff <= this.roamingRadius;
        });
        if(0 === candidates.length){
            candidates = walkableNodes;
        }
        let target = candidates[sc.randomInteger(0, candidates.length - 1)];
        if(!target){
            return false;
        }
        this.objectBody.moveToPoint({column: target.col, row: target.row});
        return true;
    }
    ...
    {
        if(this.roamingTimer){
            clearInterval(this.roamingTimer);
            this.roamingTimer = false;
            return true;
        }
        return false;
    ...
    /**
     * Loads the damage types profile of the enemy from the
     * `objects_damage_types` table. The loaded map uses the `weak_<type>` /
     * `def_<type>` key convention so the damage types system can consume it.
     * @returns {Promise<void>}
     */
    async loadDamageTypes()
    {
        this.damageTypes = {};
        let damageTypes = await this.dataServer.getEntity('objectsDamageTypes').loadByWithRelations(
            'object_id',
            this.id,
            ['related_objects']
        );
        ...
        for(let damageType of damageTypes){
            let typeKey = sc.get(damageType, 'damage_type', false);
            if(!typeKey){
                continue;
            }
            let multiplier = sc.get(damageType, 'multiplier', false);
            if(false !== multiplier && null !== multiplier){
                this.damageTypes['weak_'+typeKey] = Number(multiplier);
                continue;
            }
            this.damageTypes['def_'+typeKey] = Number(sc.get(damageType, 'defense_value', 0));
        }
    }
    ...
    startBattleWithPlayer(props)
    {
        ...
        // when enabled, a hit from a different player switches the battle target (Kaetram target switching):
        if(this.switchTargetOnHit){
            let otherPlayerId = Object.keys(this.battle.inBattleWithPlayers)
                .find((id) => Number(id) !== Number(playerSchema.player_id));
            if(otherPlayerId){
                this.battle.inBattleWithPlayers = {};
                this.battle.leaveBattle(playerSchema);
            }
        }
        this.stopRoaming();
    ...
    async onBattleEnd()
    {
        if(this.roaming && this.roamingRoom){
            this.startRoaming(this.roamingRoom);
        }
    }
