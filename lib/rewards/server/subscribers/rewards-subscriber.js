...
const { DropTablesProcessor } = require('../drop-tables-processor');
...
class RewardsSubscriber
{
    ...
    async giveRewards(playerSchema, targetObject, eventsManager)
    {
        ...
        // append the shared drop tables winning rewards:
        let dropTableRewards = await DropTablesProcessor.getWinningRewards(
            targetObject,
            playerSchema,
            eventsManager
        );
        winningRewards = [...winningRewards, ...dropTableRewards];
