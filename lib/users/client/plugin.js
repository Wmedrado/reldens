...
class UsersPlugin extends PluginInterface
{
    ...
    /**
     * Wire the two-factor (TOTP) login flow. The server requires a totpCode on
     * login when the account has two-factor enabled and returns the raw error
     * "Invalid two-factor authentication code." otherwise. This UI reads the
     * optional #totp-code input from the login form and includes it in the join
     * options, then reveals the input when the server asks for the code.
     *
     * The hidden TOTP input is defined in the theme index.html and toggled by
     * showTotpCodeOnTwoFactorError. A dedicated reactive re-render pass is not
     * needed: the existing login form handler re-submits on the next submit.
     *
     * @returns {boolean}
     */
    setupTwoFactorUi()
    {
        ...
        this.events.on('reldens.beforeJoinGame', (event) => {
            this.attachTotpCode(event);
        });
        this.events.on('reldens.gameRoomError', () => {
            this.showTotpCodeOnTwoFactorError();
        });
        return true;
    }
     ...
     * @returns {boolean}
     */
    attachTotpCode(event)
    {
        let formData = sc.get(event, 'formData', false);
        if(!formData){
            return false;
        }
        let totpInput = this.gameManager.gameDom?.getElement('#totp-code');
        if(!totpInput || '' === String(totpInput.value || '').trim()){
            return false;
        }
        let totpCode = String(totpInput.value).trim();
        formData.totpCode = totpCode;
        if(this.gameManager.userData){
            this.gameManager.userData.totpCode = totpCode;
        }
        return true;
    }
     ...
     * @returns {boolean}
     */
    showTotpCodeOnTwoFactorError()
    {
        let lastErrorMessage = this.gameManager.gameClient?.lastErrorMessage || '';
        if('Invalid two-factor authentication code.' !== lastErrorMessage){
            return false;
        }
        let container = this.gameManager.gameDom?.getElement('.login-totp');
        if(!container){
            return false;
        }
